    ...
    const handleLikeToggle = (postId) => {
        ...
        if (isLiked) {
            dispatch(PostUnlike(postId)).then(() => {
                ...
                setLikesCount(prev => ({ [postId]: (prev[postId] || 1) - 1 }));
            });
        } else {
            dispatch(PostLike(postId)).then(() => {
                setLikedPosts(prev => ({ ...prev, [postId]: true }));
                setLikesCount(prev => ({ [postId]: (prev[postId] || 0) + 1 }));
            });
        }
    };
                    ...
                    return (
                        <div key={post._id} className={getpoststyles.post}>
                            ...
                            <div className={getpoststyles.likesandcomment}>
                                    ...
                                    <span className={getpoststyles.likesandlikes} >{likesCount[post._id] || 0} 
                                        <div onClick={() => handleLikeToggle(post._id)}>
                                            {likedPosts[post._id] ? <i class="fa fa-heart" aria-hidden="true"></i> : <i class="fa fa-heart-o" aria-hidden="true"></i>}
                                        </div>
                                    </span>
